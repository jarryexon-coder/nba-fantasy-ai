    ...
    try {
      ...
      switch(selectedSport) {
        case 'NBA':
          ...
          break;
        case 'NHL':
          gamesResponse = await apiService.getNHLGames();
          break;
        case 'NFL':
          gamesResponse = await apiService.getNFLGames();
          break;
        default:
          gamesResponse = { data: [], isMock: true };
      }
      
      // Use mock data for live games (since API might not have live data)
      setGames(liveGamesData[selectedSport] || []);
      
    } catch (error) {
      console.log('Error loading live games:', error.message);
      setGames(liveGamesData[selectedSport] || []);
    } finally {
      setLoading(false);
      setRefreshing(false);
    ...
    <View key={game.id} style={styles.gameCard}>
      <View style={styles.gameHeader}>
        <View style={styles.teamColumn}>
          <View style={[styles.teamLogo, { backgroundColor: game.awayColor }]} />
          <Text style={styles.teamName} numberOfLines={1}>{game.awayTeam}</Text>
          <Text style={styles.teamRecord}>{game.awayRecord}</Text>
        </View>
        ...
        <View style={styles.scoreColumn}>
          ...
          <View style={styles.timeContainer}>
            <Text style={styles.period}>{game.period || game.quarter}</Text>
            <Text style={styles.timeRemaining}>{game.timeRemaining}</Text>
          </View>
        </View>
        
        <View style={styles.teamColumn}>
          <View style={[styles.teamLogo, { backgroundColor: game.homeColor }]} />
          <Text style={styles.teamName} numberOfLines={1}>{game.homeTeam}</Text>
          <Text style={styles.teamRecord}>{game.homeRecord}</Text>
        </View>
      </View>
      ...
      <View style={styles.gameDetails}>
        <View style={styles.detailRow}>
          ...
          <Text style={styles.detailText}>{game.arena || game.stadium}</Text>
          <Ionicons name="people" size={12} color="#6b7280" style={styles.detailIcon} />
          <Text style={styles.detailText}>{game.attendance}</Text>
          <Ionicons name="tv" size={12} color="#6b7280" style={styles.detailIcon} />
          <Text style={styles.detailText}>{game.channel}</Text>
        </View>
      ...
      <View style={styles.keyPlayers}>
        ...
        <View style={styles.playersRow}>
          {game.keyPlayers?.slice(0, 2).map((player, index) => (
      ...
      <View style={styles.statsGrid}>
        ...
        <View style={styles.statCard}>
          <Text style={styles.statNumber}>
            {games.reduce((sum, game) => sum + game.awayScore + game.homeScore, 0)}
          </Text>
      ...
      >
        ...
        <View style={styles.gamesSection}>
          <Text style={styles.sectionTitle}>
            {selectedSport} Games {games.filter(g => g.status === 'Live').length > 0 && '🔥'}
          </Text>
