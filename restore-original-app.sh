...
echo "🔄 RESTORING YOUR ORIGINAL APP FILES"
echo "===================================="
echo "This will restore your app to its working state before any fixes."

# Check if you have git
if command -v git &> /dev/null; then
    echo "Found git, checking for recent commits..."
    if git status &> /dev/null; then
        ...
        if [[ "$response" =~ ^[Yy]$ ]]; then
            ...
            echo "✅ Restored screens from git"
            exit 0
        fi
    fi
fi
...
BACKUP_FILES=$(find src/screens -name "*.backup_*" -type f | head -5)
...
if [ -n "$BACKUP_FILES" ]; then
    ...
    echo "No backup files found. Creating fresh copies from template..."
    
    # Create fresh HomeScreen
    cat > src/screens/HomeScreen-working.js << 'HOMESCREEN'
// src/screens/HomeScreen-working.js - RESTORED VERSION
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.subtitle}>Restored Version</Text>
        </View>
      </View>
...
echo "\n🎯 MANUAL FIXES NEEDED (AFTER RESTORATION):"
echo "=========================================="
echo "1. Fix HTML tags in PlayerStatsScreen:"
echo "   - Find and replace <h1> with <Text style={{...}}>"
echo ""
echo "2. Install missing dependencies:"
echo "   npx expo install react-native-linear-gradient"
echo "   npx expo install react-native-progress"
echo ""
echo "3. Test the app:"
echo "   npx expo start --clear"
echo ""
echo "4. Only after app is working, consider gentle fixes:"
echo "   - Comment out (not remove) console.log statements"
echo "   - Replace alert() calls one by one"
echo "   - Add React keys to .map() calls"
echo ""
echo "✅ Restoration complete. Your app should work now."
